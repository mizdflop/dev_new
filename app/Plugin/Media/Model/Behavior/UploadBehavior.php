 ...
 */	
	protected $_defaults = array(
		'path' => ':rootfiles:ds:model:ds:field',
		...
		foreach ($settings as $field => $options) {
			...
			if (!empty($this->settings[$Model->alias][$field]['path'])) {
				$this->settings[$Model->alias][$field]['path'] = Folder::slashTerm(String::insert($this->settings[$Model->alias][$field]['path'], array(
						'ds' => DS,'root' => WWW_ROOT,'model' => strtolower($Model->alias),'field' => strtolower($field))));
				new Folder($this->settings[$Model->alias][$field]['path'],true,0777);
			}
			...
			if (!empty($this->settings[$Model->alias][$field]['thumbnails'])) {
				foreach ($this->settings[$Model->alias][$field]['thumbnails'] as $thumb => $options) {
					...
					new Folder(Folder::slashTerm(String::insert($this->settings[$Model->alias][$field]['path']."thumb:ds{$thumb}", array('ds' => DS))),true,0777);										
				}
			}
		}	
	}	
 ...
 */	
	public function beforeSave(Model $Model) {
		...
		foreach ($this->settings[$Model->alias] as $field => $options) {
			...
			// Fix file name
			$File = new File(Folder::slashTerm($options['path']).$Model->data[$Model->alias][$field]['name']);
			$Model->data[$Model->alias][$field]['name'] = String::uuid().'.'.$File->ext();
			
			
			// Move tmp file to upload dir
			if (!move_uploaded_file($Model->data[$Model->alias][$field]['tmp_name'], $options['path'].$Model->data[$Model->alias][$field]['name'])) {
			...
			if (!empty($options['thumbnails']) /*&& in_array($Model->data[$Model->alias][$field]['type'], $this->_imageTypes)*/) {
				...
				foreach ($options['thumbnails'] as $thumb => $opts) {
					...
					$image = $imagine->open($options['path'].$Model->data[$Model->alias][$field]['name'])
								->{$method}(new Imagine\Image\Box($w, $h),$mode)
								->save($options['path'].'thumb'.DS.$thumb.DS.$Model->data[$Model->alias][$field]['name']);					
				}
			}
 ...
 */	
	protected function _deleteFilesList($Model) {
		foreach ($this->__filesToRemove as $file) {
			if (!empty($file['name'])) {
				$File = new File(Folder::slashTerm($file['dir']).$file['name']);
				$File->delete();
				if (!empty($this->settings[$Model->alias][$file['field']]['thumbnails'])) {
					foreach ($this->settings[$Model->alias][$file['field']]['thumbnails'] as $thumb => $options) {
						$File = new File(Folder::slashTerm($file['dir']).'thumb'.DS.$thumb.DS.$file['name']);
						$File->delete();
					}
				}
			}
 ...
 * @access protected
 */
	protected function _sizeToBytes($size) {
		if (is_numeric($size)) {
			return $size;
		}
		if (!preg_match('/^([1-9][0-9]*) (kb|mb|gb|tb)$/i', $size, $matches)) {
			throw new InvalidArgumentException('The max_size option format is invalid');
		}
