...
class Opauth{
	...
	public static function authenticate() {
		...
		require_once  APP . 'Config' . DS . 'opauth.php';
	...
	public static function endpoint() {
		...
		switch($Opauth->env['callback_transport']){
			case 'session':
				if (!session_id()) {
					trigger_error('Please start session automaticly', E_USER_NOTICE);
					session_start();
				}
		...
		if (array_key_exists('error', $response)) {
			trigger_error($response['error']['raw'], E_USER_ERROR);
			return false;
		}	
		/**
		 ...
		 */
			...
			if (empty($response['auth']) || empty($response['timestamp']) || empty($response['signature']) || empty($response['auth']['provider']) || empty($response['auth']['uid'])) {
				...
				throw new Exception(sprintf('Invalid auth response: %s',$reason));
			} else {
	...
	public function run(){
		...
		if (!empty($this->env['params']['strategy'])){
			...
			elseif (array_key_exists($this->env['params']['strategy'], $this->strategyMap)){
				...
				return $this->Strategy->callAction($this->env['params']['action']);
			}
			else{
